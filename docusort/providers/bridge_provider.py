...
class BridgeProvider(Provider):
    ...
    def classify(self, *, system_prompt: str, user_prompt: str, model: str,
        ...
        if not bridge.is_connected():
            # Transient: the Mac is just asleep/offline. Let the pipeline
            # leave the document in the inbox and retry rather than burning
        ...
        try:
            ...
            # The Mac was reachable but didn't answer in time (asleep
            # mid-call, model still loading, machine bogged down). Retry
            # later instead of shelving the doc.
            raise TransientProviderError(str(exc)) from exc
        except RuntimeError as exc:
            # RuntimeError here covers "client disconnected" / "gone before
            # send" from the hub — also transient connectivity, not a bad
            # request.
            raise TransientProviderError(str(exc)) from exc
