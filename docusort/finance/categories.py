...
TX_CATEGORIES = (
    ...
    "versicherung",     # risk insurance premium (Haftpflicht, KFZ, Hausrat …)
    "sparversicherung", # savings-type insurance / retirement (Lebens-, Rentenversicherung, Riester, Rürup)
    ...
    "haushalt",         # home goods, drugstore, DIY store
    "online-shopping",  # Amazon, PayPal, eBay, Klarna … (unknown goods)
    "kinder",           # Kita, school, pocket money, toys
    "reisen",           # hotels, flights, package tours
    "kreditkarte",      # credit-card settlement (card bookings not imported)
    "kredit",           # loan instalments (Darlehen, Tilgung, Ratenkredit)
    "kapital",          # savings plans, securities, building society, crypto
...
# Categories whose outflows are money moved into a product the user still
# owns (fund savings plan, endowment insurance …): shown as "gespart /
# angelegt", never as spending. User-defined categories can opt in via
# custom_categories.is_saving.
SAVING_CATEGORIES = ("kapital", "sparversicherung")
# Money coming in — a rule for these never applies to debits (a Deka refund
# must not turn the Deka savings plan into a refund).
INCOME_CATEGORIES = ("erstattung", "gehalt", "zins-dividende", "rente-zuschuss")
