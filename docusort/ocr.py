...
def _extract_pdf_pages(pdf_path: Path) -> list[str]:
    ...
    try:
        ...
        if reader.is_encrypted:
            # Scanner apps often "protect" a PDF with an empty owner
            # password — the text is there, it just needs unlocking.
            try:
                reader.decrypt("")
            except Exception as exc:  # noqa: BLE001
                logger.warning("pypdf could not decrypt %s: %s", pdf_path.name, exc)
...
def _decrypted_copy(src: Path) -> Path:
    """ocrmypdf refuses an encrypted PDF („qpdf --decrypt"). A scan app's
    empty owner password is no protection worth keeping: write an unlocked
    copy next to the temp output and OCR that instead. Returns `src`
    unchanged when it is not encrypted or cannot be opened."""
    try:
        ...
        with open(out, "wb") as fh:
            writer.write(fh)
        logger.info("OCR: %s was encrypted (empty password) — unlocked copy used", src.name)
        return out
    except Exception as exc:  # noqa: BLE001
        logger.warning("OCR: could not unlock %s: %s", src.name, exc)
        return src
...
def _run_ocrmypdf(src: Path, dst: Path, settings: OCRSettings,
    ...
    src = _decrypted_copy(src)
    cmd += [str(src), str(dst)]

    logger.info("Running OCR: %s", " ".join(cmd))
    # Hard timeout so a stuck Tesseract can't freeze the pipeline slot.
    # subprocess.run raises TimeoutExpired on expiry; the caller catches it
    # and falls back to the un-OCR'd text. Guard against a mis-set 0/None.
    timeout = settings.timeout_seconds if settings.timeout_seconds and settings.timeout_seconds > 0 else None
...
def extract_text(file_path: Path, settings: OCRSettings) -> OcrResult:
    ...
    if suffix in PDF_SUFFIXES:
        ...
        try:
            ...
            logger.error(
                "OCR timed out after %ds for %s – using un-OCR'd text",
                settings.timeout_seconds, file_path.name,
            )
    ...
    if suffix in IMAGE_SUFFIXES:
        try:
            ...
            tess_timeout = (
                ...
                if settings.timeout_seconds and settings.timeout_seconds > 0
                else 0
            )
...
def extract_pages(file_path: Path, settings: OCRSettings) -> list[str]:
    ...
    if suffix in PDF_SUFFIXES:
        ...
        try:
            ...
            logger.error("OCR timed out after %ds for %s – using un-OCR'd text",
                         settings.timeout_seconds, file_path.name)
            tmp_path.unlink(missing_ok=True)
            return pages
        except subprocess.CalledProcessError as exc:
